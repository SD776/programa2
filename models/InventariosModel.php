...
class InventariosModel extends Query{
    ...
    {
        $sql = "SELECT i.accion, i.cantidad, i.stock_Actual, i.fecha, p.descripcion, p.cantidad as cantidad_actual FROM inventario i INNER JOIN productos p ON i.id_producto = p.id WHERE i.id_producto = $idProducto AND i.id_usuario = $id_usuario";
        return $this->selectAll($sql);
    }
